...
class PolicyEngine:
    ...
    async def check_usage_limits(
            ...
            return _result(
                {
                    ...
                    "paid_remaining": plan.max_requests_per_interval
                    - billing_account.requests_used_current_period,
                    "should_upgrade": False,
                }
            )

        if plan.free_trial_days > 0 and billing_account.trial_started_at:
            trial_end = billing_account.trial_started_at + timedelta(
        ...
        paid_remaining = (
            plan.max_requests_per_interval - billing_account.requests_used_current_period
        )
        if paid_remaining > 0:
            return _result(
                {
